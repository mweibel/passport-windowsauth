...
var debug = require('debug')('passport-windowsauth:ldapvalidator');
...
LdapValidator.prototype.validate = function (username, password, callback) {
  debug('validate', username, password !== '');
  if (!username) {
    return callback();
  }
  ...
  this._lookup.search(username, function (err, up) {
    ...
    var client = this._options.binder || ldap.createClient({ url: this._options.url, tlsOptions: this._options.tlsOptions, reconnect: true });
